...
def extrair(filial, arquivo):
    try:
        ...
        # SQL AMPLIADO: Pegando todas as operações de saída 'S' para bater com a 322
        query = f"""
            SELECT 
            ...
            WHERE P.CODEPTO = 105 
              ...
              AND M.CODOPER IN ('S', 'E', 'ED', 'ST', 'SM') -- Adicionado ST e SM para garantir
...
if __name__ == "__main__":
    print("🤖 RotiFácil: Sincronizador de Precisão Ativado")
    while True:
        if extrair(2, "vendas_filial2.csv") and extrair(5, "vendas_filial5.csv"):
            subir(["vendas_filial2.csv", "vendas_filial5.csv"])
